...
public class TweetService {
	...
	//takes a list of tweets and pattern matches to find the link 
	private void shortenLinks(List<Tweet> tweets) {
		...
		for(Tweet tweet: tweets) {
			...
			Matcher matcher = pattern.matcher(message);
			while(matcher.find()) {
				//creates a shortened link if link length is greater than 23 chars
	...
	private List<TweetDisplay> formatTimestamps(List<Tweet> tweets){
		...
		for(Tweet tweet : tweets) {
		    ...
		    //if its greater than 3 days display one way
			if (diff > 3) {
